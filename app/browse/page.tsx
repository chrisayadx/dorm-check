...
export default function BrowsePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (dormsRes.error) {
        console.error('Dorm load error:', dormsRes.error)
      ...
      if (reviewsRes.error) {
        console.error('Review load error:', reviewsRes.error)
        setReviews([])
      } else {
  ...
  const grouped = useMemo(() => {
    ...
    for (const dorm of dorms) {
      const university =
        dorm.university || 'Unknown University'

      const bucket = map.get(university)

      if (bucket) {
        bucket.push(dorm)
      } else {
        map.set(university, [dorm])
      }
    }
  ...
  const shown = active
    ? grouped.filter(([university]) => university === active)
    : grouped
      ...
      <section>
        <SiteFrame media={<CampusGround seed="browse" />}>
          <div className="hero-lead">
            ...
            >
              Pick a university to narrow the list, or
              scroll through everything students have
              added so far.
            </p>
          </div>
        </SiteFrame>
      ...
      >
        ...
        {!loading &&
          ...
          shown.map(([university, list]) => {
            const code = initialsOf(university)
        ...
        {!loading && loadFailed && (
          ...
          >
            ...
            >
              The database did not answer. Nothing is
              missing, it just could not be fetched.
            </p>
        ...
        {!loading &&
          ...
          dorms.length === 0 && (
            ...
            >
              ...
              >
                Add the first building and it shows up
                here straight away.
              </p>
